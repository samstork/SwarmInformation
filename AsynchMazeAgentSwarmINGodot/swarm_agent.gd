...
enum State {EXPLORING, BEACON, SUBGOAL, SUBGOAL_F, BACKTRACK_B, BACKTRACK_G, FOLLOW}
...
@export_range(0, 1) var wall_avoid_wt_explore: float = 0.35
@export_range(0, 1) var wall_avoid_wt_exploit: float = 0.35
@export var sgf_timeout = 150
var SGF_counter = 0
...
var current_state: State:
	...
	set(value):
		...
		match value:
			...
			State.BACKTRACK_B, State.BACKTRACK_G:
...
func _physics_process(delta: float) -> void:
	...
	match current_state:
		State.EXPLORING:
			if len(visible_beacons) == 0 and beaconing:
				self.current_state = State.BACKTRACK_B
			elif visible_goals:
				Globals.goal_sighted.emit()
				...
				if ray_left.is_colliding():
					direction_angle += wall_avoid_wt_explore * PI
		...
		State.FOLLOW:
			...
			if ray_left.is_colliding():
				direction_angle += wall_avoid_wt_exploit * PI
			if ray_right.is_colliding():
				direction_angle -= wall_avoid_wt_exploit * PI
		...
		State.SUBGOAL_F:
			...
			if visible_exploiters:
				SGF_counter = 0
			if SGF_counter > sgf_timeout:
				current_state = State.FOLLOW
				
		

func become_subgoal() -> void:
